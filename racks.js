    ...
    var RacksJS = (function () {
            ...
            requestObject = http.request(options, function (reply) {
                ...
                reply.on('end', function () {
                    try {
                        ...
                        if (_racks.verbosity > 0) {
				console.log('reply was not JSON. Raw reply:', replyBody);
			}
			callback();
        ...
        RacksJS.prototype.authenticate = function (authObjectRequest, callback) {
            ...
            _racks.request({
                ...
                if (authObjectReply.access === undefined) {
                    console.log('Api replied:', authObjectReply);
                    callback('auth failed - check username and apikey');
                } else {
        ...
        RacksJS.prototype.products = {
            ...
            'cloudServersOpenStack': {
                ...
                'images': {
                    model: function (_racks, product, resource) {
                        resource.deleteImage = function (callback) {
                            _racks.request({
                                method: 'DELETE',
                                url: resource.links[0].href
                            }, function (reply) {
            ...
            'cloudDNS': {
                'domains': {
                    model: function (_racks, product, resource) {
                        resource.listSubdomains = function (callback) {
                          _racks.request({
                            method: 'GET',
                            url: resource.target + '/subdomains'
                          }, function (reply) {
				callback(reply);
			  });
                        };
			resource.records = function (callback) {
			  _racks.request({
				method: 'GET',
				url: resource.target + '/records'
			  }, function (reply) {
				callback(reply.records);
			  });
			};
			return resource;
                      }
